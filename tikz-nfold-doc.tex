...
\begin{tkzexample}[latex=3.75cm]
\begin{tikzpicture}
  \draw[black] (1,0) -- (3,0) -- (1.5,1.2) -- (4,0.8);
  \draw[red, line width=1pt, double distance=.6cm, nfold]
    (1,0) -- (3,0) -- (1.5, 1.2) -- (4,0.8);
  \draw[blue, double distance=.6cm, nfold] (1,0) -- (3,0)
    to[relative, out=1, in=179] (1.5, 1.2) -- (4,0.8);
\end{tikzpicture}
\end{tkzexample}
...
\begin{itemize}
  ...
  \item \verb|\pgfoffsetpathindex{softpath}{width}{i}{n}|: In this convenience method, \texttt{i} and \texttt{n} are integers with $1 \leq i \leq n$. It calls the previous macro with \texttt{fraction=-1} for \texttt{i=1} and with \texttt{fraction=1} for \texttt{i=n} (with all other values interpolated), and is capable of reproducing the output of \nfold{} (albeit in a less efficient way).
\end{itemize}
