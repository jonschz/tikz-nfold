...
\end{tkzexample}
  ...
  \item Very short curves with large angles at the ends result in a glitched output:
\begin{tkzexample}[latex=3.5cm]
\begin{tikzpicture}
  \draw[black] (1, 0) -- (3,0) -- (2, 1.2) -- (4, .8);
  \draw[blue, double distance=.5cm, nfold] (1, 0) -- (3,0)
     to[relative, out=1, in=179] (2, 1.2) -- (4, .8);
\end{tikzpicture}
\end{tkzexample}
  This issue has been fixed for straight lines in version \texttt{0.0.2}, but it is much harder to fix for curves.
...
\begin{itemize}
  ...
  \item \textbf{0.0.2}: Bugfixes
  \begin{itemize}
    \item \texttt{dashed} no longer influences the arrow tip
    \item Very short lines with large angles at both ends are now offset correctly
  \end{itemize}
\end{itemize}
