...
This feature can be used to draw multiple instances of a path with different settings. Use \texttt{/tikz/nfolder distance=<hwidth>} to specify the large path's half width. Activate the feature using \texttt{/tikz/nfolder=\{style\_1, ..., style\_k\}} and provide a list of styles for the different constituent paths.
\begin{tkzexample}[latex=4cm]
\begin{tikzpicture}
  \draw[nfolder distance=6pt,nfolder={red,green,blue,violet}]
    (0,0) -- (2,0) arc(-90:90:1)
    to[out=180, in=0] (0,1) -- (0,2);
\end{tikzpicture}
\end{tkzexample}
This is also supported in \texttt{tikz-cd}:
\begin{tkzexample}[latex=3.5cm]
\begin{tikzcd}
  a \ar[rr, out=10, in=190, nfolder distance=2mm, nfolder=
    {, -Latex, {red, thick, Stealth-Latex}, Latex-}] & & b
\end{tikzcd}
\end{tkzexample}
As this feature has only been implemented recently, bugs are likely to be present. Please report them if you find any.

\subsection{offset}
The key \texttt{/tikz/offset=<length>} can be used to offset a single path:
\begin{tkzexample}[latex=2.5cm]
\begin{tikzpicture}
  \draw[offset=-1mm] (0,0) to[bend left] (2,0);
  \draw[offset=1mm] (0,0) to[bend right] (2,0);
\end{tikzpicture}
\end{tkzexample}
This also works with \texttt{fill}:
\begin{tkzexample}[latex=2.5cm]
\begin{tikzpicture}
  \draw[offset=-2mm, fill=blue] (0,0) to[bend left]
    (2,0) -- (2,1) -- (0,1) -- cycle;
\end{tikzpicture}
\end{tkzexample}
