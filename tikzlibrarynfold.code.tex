...
\def\pgf@nfold@handlesegment{%
  ...
  \ifx\pgf@nfold@currentinputsegment\pgf@nfold@inputsegmentclosepath
    ...
    \pgfpathclose
% Old code, likely no longer needed
%      \ifpgf@nfold@continuesegment
%        \pgfoffsetlinenomove{\pgf@nfold@segment@start}{\pgf@nfold@segment@end}{\pgf@shiftdec@amount}
%      \else
%        \pgfoffsetline{\pgf@nfold@segment@start}{\pgf@nfold@segment@end}{\pgf@shiftdec@amount}
...
% Mostly for completeness sake; using TikZ' (0,0) rectangle (1,1) does not call this code
...
\def\pgf@nfold@processnextinputsegmentobject{%
  ...
  % TODO possible optimisation: Reuse some values computed for the next segment
  \pgf@nfold@computeangles%
...
\def\pgfoffsetpath#1#2{%
  \begingroup
    \pgfmathsetlengthmacro\pgf@decoration@nfold@hwidth{#2}
    % \pgf@decoration@nfold@hwidth must always be positive
    \pgf@x=\pgf@decoration@nfold@hwidth\relax
    ...
    \else
      \def\pgf@shiftdec@fraction{1}
    \fi
    \edef\pgf@decoration@nfold@hwidth{\the\pgf@x}
    \pgfoffsetpathqfraction{#1}{\pgf@decoration@nfold@hwidth}{\pgf@shiftdec@fraction}
  \endgroup
}
...
\def\pgfoffsetpathqfraction#1#2#3{%
  \begingroup
    ...
    \edef\pgf@shiftdec@amount{\the\pgf@x}
    \pgf@nfold@traversepath%
  \endgroup
}
