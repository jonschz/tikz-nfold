...
\def\pgf@nfold@handlesegment@testoffset{%
  ...
  % Step 2.2: Store where the current end point was relocated
...
\pgfkeys{
  ...
  /tikz/scaling nfold/.code={%
    \pgfscope% scope to contain \tikz@double@setup
      \tikz@double@setup
      % extract double distance between line centers
      \pgf@x=\pgfinnerlinewidth\relax
      \advance\pgf@x by\pgflinewidth\relax
      \pgf@x=.5\pgf@x
      \c@pgf@counta=#1
      % multiply by (order-1)
      \advance\c@pgf@counta by -1\relax
      \global\pgf@x=\c@pgf@counta\pgf@x
    \endpgfscope
    \pgfkeys{
      /tikz/double distance between line centers=\pgf@x,
      /tikz/nfold=#1
    }
  },
}
