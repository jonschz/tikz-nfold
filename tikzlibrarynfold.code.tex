...
\def\pgf@nfold@roundjoin{
  ...
  \ifdim\insidepercentage pt<.5pt\relax
    ...
    % TODO can this code be simplified before we refactor the \pgfmathparse calls?
...
\def\pgf@nfold@make@join{
  ...
  \ifdim\pgf@xa>0.1pt\relax
    ...
    \pgfmathsetmacro{\insidepercentage}{-.5*\turnindicator*\pgf@nfold@shift@fraction+.5}
    \if m\pgf@cached@linejoin% \ifx is not needed because both are only one character
      ...
      \pgf@xa=\pgf@nfold@deltaphi pt\relax
      \pgf@xa=.5\pgf@xa
...
\def\pgf@nfold@handlesegment{%
  ...
  % when we draw the join at the start of the next segment
