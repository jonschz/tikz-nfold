...
% TODO make a short comment documentation out of this
% Sketch what I want to achieve:
% - one macro that describes the entire path
% - we can set a global fraction variable to affect how the macro offsets the path when called
% - this macro can contain the following types of calls:
%   - moveto's, lineto's, curveto's, closepath's, rect's
%   - these should already be shortened for the joins
%   - miterjoin, roundjoin, beveljoin
%     - question: should the join type be set on the fly? Probably no (changed my mind)
%       - better efficiency if we compute as little as possible in the join itself
%
% What information do we need?
% - lines, curves: control points + tangents (or normals?) for efficiency; angles?
%   - for curves: 6 points/vectors; cannot do 12 parameters
%   - instead: either set in the same macro or use \pgfqpoint / \pgf@x=...
% - joins: start angle, end angle, first point; could consider tangents for efficiency
...
%  \pgfoffsetline{#1}{#2}{\pgf@nfold@shiftamount}%
  \pgfoffsetlinenomove{#1}{#2}{\pgf@nfold@shiftamount}%
}
...
\def\pgf@nfold@parse@setupfirst{%
  % Status variables
  % types: f=first, m=moveto, c=curveto, l=lineto, r=rect, o=closepath, t=last
...
\def\pgfoffsetpathqfraction#1#2#3{%
  \begingroup
    ...
    \pgf@nfold@parsesoftpath{\cachedpath}{\parsedsoftpath}
%    \pgfwarning{new path: \meaning\pgf@nfold@parsedpath}
  % Transformations are already baked into the path; without this call, they would be applied twice
    \pgftransformreset%
    \pgf@nfold@parsedpath
  \endgroup
