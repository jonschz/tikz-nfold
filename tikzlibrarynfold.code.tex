...
% Notes / TODO
% ------------
% - minor deviation in squiggly example
%
%
% Future
% ------
% - closepath implementation:
% - closepath test cases:
%   - close joins edge case at the start/end/both
% - possible optimisations
%   - the A1A4 length is computed for every offset; this is something we could cache from the subdivision phase
%   - faster, less precise veclen@ -> this is used a lot
%     - if x > y, compute x*(1+(y/x)^2)^(1/2) approx x*(A+B*(y/x)^2); optimize A=1. and B=.5 around the unit circle
...
% the structure of a pgf soft path, but it consists of different tokens.
%
...
% TODO WIP: Handling closepath


% TODO migrate modifications to other macro
\def\pgf@nfold@addmacro@new#1{%
...
% Idea
% ----
% - normal non-closed path: movetotoken{x1}{y1} linetotoken {x2}{y2} ...
% - closed path: change into movetotoken{x0}{y0} invisiblelinetotoken{x1}{y1} linetotoken{x2}{y2} ...
%
%
%
% In order to correctly implement \pgfpathclose, we need to parse the entire path twice, the reason being that we already need to know about the \pgfpathclose (and its point of origin) at the beginning of this connected segment in order to correctly draw the first/last join.
% In the first pass we split the path into connected segments and collect the necessary data. Once we have found a connected segment, we execute the second pass on that segment.
%
% TODOs
% - test path without a moveto at the beginning
%
%
% Ideas for improvements:
% - maybe we can skip parsing twice? Instead of adding the tokens back in, we could add in some macros which
%   take over the functionality of the existing parser macros.
%   - problem: curveto's take way more parameters, will prob. get ugly
% - can we optimise away that the second parser is called once with only the first moveto?
%   - will prob. get difficult if we also want to treat paths without a first moveto correctly
%
\def\pgf@nfold@parsetest#1{%
  \let\my@conn@path\pgfutil@empty%
  \let\all@segments\pgfutil@empty%
%  \let\last@moveto@pt\pgfutil@empty%
  \edef\last@moveto@pt{{\the\pgf@path@lastx}{\the\pgf@path@lasty}}%
  \let\last@closepath@from\pgfutil@empty%
  \expandafter\pgf@nfold@@parsetest#1\pgf@stop{}{}%
}%

\def\pgfsyssoftpath@nfold@invisibleline#1#2{invisiblelinetoken} % make sure this \ifx-equal only to itself

\def\handlecomplete{%
  % Use a group to avoid overwriting commands like \pgf@next;
  % might also be beneficial for memory usage
  \begingroup%
    % save the current macro so we can add it to both the current and the next connected segment
    \let\my@tmp@stuff\pgf@nfold@macrotoadd
    % Add in the beginning of the path
    \ifx\last@closepath@from\pgfutil@empty
      \edef\path@start{\noexpand\pgfsyssoftpath@movetotoken\last@moveto@pt}%
    \else
      % TODO first hacky approach, incomplete
      \edef\path@start{%
        \noexpand\pgfsyssoftpath@movetotoken\last@closepath@from%
        \noexpand\pgfsyssoftpath@nfold@invisibleline\last@moveto@pt%
      }%
    \fi
    % add the start of the path
    \let\pgf@nfold@macrotoadd\my@conn@path%
    \pgf@nfold@addmacro@new\path@start%
  %  \pgfwarning{passing connected path: \meaning\my@conn@path}%
    \pgfwarning{new approach: \meaning\path@start}%
  %  \pgf@nfold@parsesoftpath{\my@conn@path}{\pgf@nfold@macrotoadd}%
    \pgf@nfold@parsesoftpath{\path@start}{\pgf@nfold@macrotoadd}%
    \pgf@nfold@addmacro@new\all@segments%
  \endgroup%
}

\def\pgf@nfold@@parsetest#1#2#3{%
  \let\pgf@next\pgf@nfold@@parsetest%
  \ifx#1\pgf@stop%
    \handlecomplete%
    \let\pgf@next\relax%
  \else%
    % pass on the unmodified macro
    \def\pgf@nfold@macrotoadd{#1{#2}{#3}}%
    \pgf@nfold@addmacro@new\my@conn@path%
    % TODO this does not have well-defined behaviour if the path does not begin with a moveto;
    % maybe I should initialise \last@moveto@pt to the previously moved-to pgf point instead
    % Other option: Check if \my@conn@path is empty?
    % Yet another alternative: Try to kill the first three tokens in my@conn@path
    \ifx#1\pgfsyssoftpath@movetotoken%
      % A moveto marks the beginning/end of one connected segment
      \handlecomplete%
      \let\my@conn@path\pgfutil@empty%
      \let\last@closepath@from\pgfutil@empty%
      \def\last@moveto@pt{{#2}{#3}}%
    \else%
      \ifx#1\pgfsyssoftpath@closepathtoken%
        \let\last@closepath@from\previous@pt%
...
\def\pgf@nfold@parser@handlesegment{%
  % Debug output
%  \pgfwarning{previous type: \meaning\pgf@nfold@prev@type; current type: \meaning\pgf@nfold@cur@type; next type: \meaning\pgf@nfold@next@type}
        ...
        \ifdim\pgf@xb>0.5pt\relax
          ...
          \if\pgf@nfold@cur@type c
            ...
            \else
              ...
              \let\pgf@nfold@inputsegment@supporta\pgf@nfold@cur@movedfirst
            \fi
          \fi
        \fi
      \fi
        ...
        \else%
          ...
          \else%
            \ifx#1\pgfsyssoftpath@rectcornertoken%
              \let\pgf@next\pgf@nfold@parserect%
            \else%
              \ifx#1\pgfsyssoftpath@nfold@invisibleline%
                \let\pgf@next\pgf@nfold@parseinvisibleline%
              \else%
                \pgfutil@packageerror{tikz-nfold}{Unrecognised soft path token `#1'}{}%
              \fi%
            \fi%
...
\def\pgf@nfold@parselineto#1#2{%
  \pgf@process{\pgfpointdiff{\pgf@nfold@cur@last}{\pgf@x#1\pgf@y#2}}%
...
\def\pgf@nfold@parselast{%
  ...
  \let\pgf@nfold@next@visible=0%
  \let\pgf@nfold@next@last\pgfpointorigin% TODO probably not needed
  \pgf@nfold@parser@handlesegment%
  % do not continue parsing here because the path is at its end
%  \pgf@nfold@@parsesoftpath%
}
...
\def\pgf@nfold@parserect#1#2\pgfsyssoftpath@rectsizetoken#3#4{%
  ...
  \let\pgf@nfold@@parsesoftpath\pgf@nfold@parser@moveup%
...
% adds the contents of \pgf@nfold@macrotoadd to the path
% TODO here we could implement closepath's and detecting connected segments
\def\pgf@nfold@addmacro{%
  % need a \gdef because we have nested groups in the subdivision
  \expandafter\expandafter\expandafter\gdef%
  \expandafter\expandafter\expandafter\pgf@nfold@parsedpath%
  \expandafter\expandafter\expandafter{%
  \expandafter\pgf@nfold@parsedpath\pgf@nfold@macrotoadd}
}

%
% Tokens for the parsed path
% TODO rename; these aren't actually tokens
%

%#3: normalised tangent (for efficiency)
...
%  \pgfoffsetline{#1}{#2}{\pgf@nfold@shiftamount}%
...
% #5, #6: normalised tangents at start and end (for efficiency)
\def\pgf@nfold@token@curveto#1#2#3#4#5#6{%
%  \pgfwarning{curve: #1 #2 #3 #4 tangents: #5 #6}
%  \pgfpathmoveto{#1}%
...
% todo check if visible, skip a bunch of commands if not
...
%    \pgf@nfold@parsesoftpath{\pgf@nfold@cachedpath}{\parsedpath}
    \pgf@nfold@parsetest{\pgf@nfold@cachedpath} % TODO refactor
%    \pgfwarning{new path: \meaning\pgf@nfold@parsedpath} % Debug output
    \pgf@nfold@run@loop%
  \endpgfscope%
}
