  ...
  \else%
    ...
    \if\pgf@nfold@next@visible1
      ...
      % to zero to prevent glitches (a zero length segment has no well-defined angle anyway)
			% TODO this might be replaced by code handling the more general case, see further TOOD comments below
      \if\pgf@nfold@next@type o
        \pgf@process{\pgfpointdiff{\pgf@nfold@next@last}{\pgf@nfold@next@first}}%
...
\def\pgf@nfold@parseinvisibleline#1#2{%
  ...
  \else%
    % TODO fake a line with the same angle as the last segment. This requires a change in architecture
    % because right now, the tangents are computed during processing, not parsing. However, in this case
    % we need to know the tangents before processing.
  \fi%
}%
